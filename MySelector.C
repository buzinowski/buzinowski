...
if (entry <0 )    Abort("ENTRY < 0");
if (entry == 0) cout<<"Filling histogram"<<endl;
cout<<".";
for (Long64_t itracks=0;itracks<n_tracks_mc;itracks++)
   {
      
      if(TMath::Abs(pt_mc[itracks])<3 && TMath::Abs(eta_mc[itracks]) < 1.5 &&  mother_ID_mc[itracks] == -1 ) 
      {
        
         if(PDG_code_mc[itracks]==2212)//protons
         {
            h_pt_mc->Fill(TMath::Abs(pt_mc[itracks]));
...
void MySelector::Terminate()
{
   ...
   #include <TCanvas>
   h_eff->Divide(h_pt_mpd,h_pt_mc);
   TCanvas* c1 = new TCanvas("c1","Histogram of Pt MPD",600,800);
   h_pt_mpd->Draw("coltz");
   TCanvas* c2 = new TCanvas("c2","Histogram of Pt MC",600,800);
   h_pt_mc->Draw("coltz");
   TCanvas* c3 = new TCanvas("c3","Histogram of Efficiency",600,800);
   h_eff->Draw("coltz");
